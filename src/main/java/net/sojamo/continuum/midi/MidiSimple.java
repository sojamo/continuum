...
public class MidiSimple {
  ...
  public MidiSimple( int theDeviceId, Receiver theReceiver ) {
    print();
    try {
      MidiDevice device = MidiSystem.getMidiDevice( MidiSystem.getMidiDeviceInfo()[theDeviceId] );
      device.open();
      Transmitter conTrans = device.getTransmitter();
      conTrans.setReceiver( theReceiver );
    } 
    catch ( MidiUnavailableException e ) {
      System.out.println("Sorry midi controller with Id : "+theDeviceId +" doesnt want to connect.");
      e.printStackTrace();
    } 
    catch (NullPointerException e) {
      System.out.println("No Midi device ( "+theDeviceId+" ) is available.");
    }
  }
  ...
  public MidiSimple( String theDeviceName, Receiver theReceiver ) {
    ...
    try {
      ...
      System.out.println("Sorry midi controller "+theDeviceName +" doesnt want to connect.");
      e.printStackTrace();
    } 
    catch (NullPointerException e) {
  ...
  public MidiSimple( int theDeviceId ) {
    new MidiSimple(theDeviceId, new MidiInputReceiver( ""+theDeviceId ) );
  }
